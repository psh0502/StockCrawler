    ...
    public class StockPriceUpdateJobTest : UnitTestBase
    {
        ...
        public void StockPriceUpdateTest()
        {
            ...
            using (var db = new StockDataContext(ConnectionStringHelper.StockConnectionString))
            {
                ...
                Assert.AreEqual(1158, data.Count);
                Assert.IsTrue(data.Where(d => d.StockNo == TEST_STOCKNO_台積電).Any());
            }
            using (var db = new StockDataContext(ConnectionStringHelper.StockConnectionString))
            {
                int? pageCount = null;
                var data = db.GetStockPriceHistoryPaging(TEST_STOCKNO_台積電, SystemTime.Today, SystemTime.Today, 1, 1, 100, ref pageCount).ToList();
                ...
                data = db.GetStockPriceHistoryPaging("0000", SystemTime.Today, SystemTime.Today, 1, 1, 100, ref pageCount).ToList();
                Assert.AreEqual(1, data.Count);
                Assert.AreEqual(1, pageCount);
                d1 = data.First();
                _logger.DebugFormat("StockNo={0}\r\nStockDT={1}\r\nOpenPrice={2}\r\nHighPrice={3}\r\nLowPrice={4}\r\nClosePrice={5}\r\nVolume={6}\r\nDeltaPrice={7}\r\nDeltaPercent={8}%\r\nPE={9}",
                ...
                Assert.AreEqual(136193381413, d1.Volume);
